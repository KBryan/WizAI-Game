...
def get_repo_url() -> str:
    """Get GitHub repository URL from git remote or ADW_REPO env var."""
    # Check ADW_REPO env var first (allows repo-agnostic operation)
    adw_repo = os.getenv("ADW_REPO")
    if adw_repo:
        # ADW_REPO should be in owner/repo format, convert to URL
        if "/" in adw_repo and not adw_repo.startswith("http"):
            return f"https://github.com/{adw_repo}"
        return adw_repo
    ...
    try:
        ...
        raise ValueError(
            "No git remote 'origin' found and ADW_REPO not set. "
            "Please set ADW_REPO environment variable (e.g., ADW_REPO=owner/repo) "
            "or run from a git repository with a remote."
        )
